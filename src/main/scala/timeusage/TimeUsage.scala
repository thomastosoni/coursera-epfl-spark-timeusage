...
object TimeUsage extends TimeUsageInterface:
  ...
  def row(line: List[String]): Row =
    val typedLines = line.head :: line.tail.map(_.toDouble)
    Row.fromSeq(typedLines)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    columnNames.foldLeft((List.empty[Column], List.empty[Column], List.empty[Column])) { case ((primaryNeeds, work, other), name) =>
      if primaryNeedsPrefixes.exists(prefix => name.startsWith(prefix)) then
        (col(name) :: primaryNeeds, work, other)
      else if workPrefixes.exists(prefix => name.startsWith(prefix)) then
        (primaryNeeds, col(name) :: work, other)
      else if otherPrefixes.exists(prefix => name.startsWith(prefix)) then
        (primaryNeeds, work, col(name) :: other)
      else
